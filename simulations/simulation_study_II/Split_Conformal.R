  ...
  for (causal in c(0.01,0.1,0.5)) {
    for (phi in c(0.05,0.15)) {
      ...
      for (rep in 1:10) {
        ...
        model = lm(pheno ~ PGS+SEX+AGE+PC1, data = calib.data)
        ...
        test.model = lm(pheno ~ PGS+SEX+AGE+PC1, data = test.data)
        ...
        write.table(test.pheno, paste0("prediction_interval/Split_Conformal/","h2_", h2, "_poly_", causal, "_phi_", phi, "_PI_test_rep_", rep, ".txt"), 
